...
public class Polygon extends Sprite
{
    ...
    public
    Polygon(int x, int y, int w, int h, Color color)
    {
        super(
            x - (w >> 1),
            y - (h >> 1),
    ...
    public
    Polygon(int x, int y, int w, int h, Color color, int vertices_num)
    {
        super(x, y, w, h, color);
    ...
    draw_lines(Graphics graphics)
    {
        ...
        for (int i = 1; i < vertices_x.length; i++) {
            graphics.drawLine(
                vertices_x[i],
                vertices_y[i],
                vertices_x[i - 1],
